...
export default async function BillDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mt-4 space-y-4">
      <Link href="/books/bills" className="inline-block text-sm font-medium text-stone-500 hover:text-stone-800">
        ← Bills
      </Link>
      ...
      {bill.is_voided && voidedBy !== null && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800">
          ...
          <Link href={`/books/bills/${voidedBy.id}`} className="font-medium underline">
      ...
      {bill.is_reversal && bill.reverses_id !== null && (
        <div className="rounded-xl border border-violet-200 bg-violet-50 p-3 text-sm text-violet-800">
          ...
          <Link href={`/books/bills/${bill.reverses_id}`} className="font-medium underline">
      ...
      <section className={cardCls}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Link
            href={`/books/vendors/${bill.vendor_id}`}

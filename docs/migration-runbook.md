...
3. `approval_requests_payment_kind.sql`
4. `approval_requests_payment_fields.sql`
5. `approval_trail.sql`
6. `stock_adjustment_approvals.sql`
7. `recipe_versions.sql`
8. `pos_sync_runs.sql`
9. `pos_credentials.sql`
10. `accounting_journal.sql`
11. `restaurant_invitations.sql`
12. `purchase_quotes.sql`
13. `salary_structures.sql`
14. `staff_holidays.sql`
15. `views_security_invoker.sql`
16. `stock_transfers.sql`
17. `recurring_journal_entries.sql`
18. `accruals.sql`
19. `fixed_assets.sql`
20. `pos_statement_imports.sql`
21. `payroll_statutory_config.sql`
22. `opening_balance_batches.sql`
23. `production_recipe_versions.sql`
24. `purchase_invoice_matches.sql`
25. `leave_policies.sql`
26. `attachment_lifecycle.sql`
27. `production_yield_variance.sql`
28. `stock_lots.sql`
29. `leave_requests.sql`
30. `recipe_line_substitutions.sql`
31. `production_variance_reviews.sql`
32. `pos_reconciliation_reviews.sql`
33. `leave_carry_forward_ledger.sql`
34. `mobile_mutations.sql`
35. `demo_environment.sql`
36. `financial_ledger_views.sql`
37. `permission_freezes.sql`
...
workflow is enabled: `attachments_rls.sql`, `btree_gist_out_of_public.sql`,

...
async function main() {
  ...
  const vCashier = await saveVoucher({
    date: D1, amount: '300', paidTo: 'Zz Veg Vendor', paidBy: 'cashier', ownerName: '', category: 'general', note: 'zz cash smoke',
  })
  assert.ok(vCashier.ok, `cashier voucher failed: ${vCashier.ok === false ? vCashier.error : ''}`)
  const vOwner = await saveVoucher({
    date: D1, amount: '1000', paidTo: 'Zz Gas Agency', paidBy: 'owner', ownerName: 'Zz Asheel', category: 'gas', note: '',
  })
  assert.ok(vOwner.ok, `owner voucher failed: ${vOwner.ok === false ? vOwner.error : ''}`)
  assert.equal(vOwner.voucher.owner_name, 'Zz Asheel')

  const noName = await saveVoucher({ date: D1, amount: '50', paidTo: 'X', paidBy: 'owner', ownerName: '', category: 'general', note: '' })
  assert.ok(!noName.ok && /which owner/i.test(noName.error))
  const ownerReimb = await saveVoucher({ date: D1, amount: '50', paidTo: 'X', paidBy: 'owner', ownerName: 'Zz Asheel', category: 'owner_reimbursement', note: '' })
  ...
  const reimb = await saveVoucher({
    date: '2001-02-05', amount: '400', paidTo: 'Zz Asheel', paidBy: 'cashier', ownerName: '', category: 'owner_reimbursement', note: '',
  })

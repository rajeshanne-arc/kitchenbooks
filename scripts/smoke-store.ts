...
async function main() {
  ...
  assert.equal(sections.length, 8, 'expected the 8 seeded sections')
  ...
  assert.equal(plt1.on_hand_qty, '20')
  ...
  assert.equal(issue.stock[0].on_hand_qty, '15')

  // stock page + section page agree
  const stockRows = await listStock(restaurant.id, 'PLT-001')
  assert.equal(stockRows[0].on_hand_qty, '15')
  assert.equal(stockRows[0].issued_qty, '5')
  ...
  assert.equal(waste.stock.on_hand_qty, '14')
  ...
  assert.equal(wVoid.stock.on_hand_qty, '15')
  ...
  assert.equal(iVoid.stock[0].on_hand_qty, '20')
  ...
  assert.equal(finalStock.on_hand_qty, '20')

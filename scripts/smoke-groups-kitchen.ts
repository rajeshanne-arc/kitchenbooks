...
async function main() {
  ...
  const wrongSection = await saveIssue({ issueDate: D1, sectionId: td.id, lines: [{ itemId: plt1.id, qty: '1', note: '' }], indentId: ind1.indent.id, session: 'Morning' })
  assert.ok(!wrongSection.ok && /belongs to/i.test(wrongSection.error), 'section mismatch is refused, named')
  const issue1 = await saveIssue({ issueDate: D1, sectionId: ch.id, lines: [{ itemId: plt1.id, qty: '1.5', note: '' }], indentId: ind1.indent.id, session: 'Morning' })
  ...
  const again = await saveIssue({ issueDate: D1, sectionId: ch.id, lines: [{ itemId: plt1.id, qty: '1', note: '' }], indentId: ind1.indent.id, session: 'Morning' })

...
const { businessYesterday } = await import('../src/server/business-day')
  ...
  const badMap = await mapPosItem({ posItemId: 'ZZT-1', itemName: 'Zz Paneer Tikka', recipeId: rid })
  ...
  const mapRes = await mapPosItem({ posItemId: 'ZZT-1', itemName: 'Zz Paneer Tikka', recipeId: dish.id })
  ...
  const remap = await mapPosItem({ posItemId: 'ZZT-1', itemName: 'Zz Paneer Tikka', recipeId: dish.id })
  assert.ok(remap.ok)
